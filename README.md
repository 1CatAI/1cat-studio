...
它把 **1Cat-vLLM 模型部署 → 对话与 Agent → 创作与交付 → GPU 监测** 连成一套日常工作流程。你可以从已有的模型和推理环境开始，也可以通过安装引导准备环境、下载适配模型，再保存自己的启动预设。
...
导入模型、选择推理环境和 GPU、启动服务之后，就可以在同一个界面里聊天、使用 Agent，或把模型接入自己的应用。启动进度、日志、下载任务和失败重试都有对应入口，常用配置可以保存为预设。
...
| 模块 | 可以做什么 |
| --- | --- |
| **模型与推理环境** | 导入本地权重；从适配目录下载模型；安装或导入 1Cat-vLLM 环境；保存启动预设；选择 GPU、TP、精度、量化与上下文；查看启动日志、停止或切换模型。 |
