...
// Adapted from basketikun/infinite-canvas @ d213a74614e0e4bd8a26383d1e1e907249e9c61b.
// Copyright (c) 2026 basketikun. See studio/third_party/infinite-canvas/LICENSE.
// Studio changes: theme tokens, bounded zoom, node controls, keyboard focus and pointer cleanup.
import React, { useEffect, useRef, useState } from "react";

import type { Viewport as ViewportTransform } from "./types";
type CanvasBackgroundMode = "lines" | "dots" | "blank";
const theme = {
	canvas: {
...
export function InfiniteCanvas({
	...
	useEffect(() => {
		const handleKeyDown = (event: KeyboardEvent) => {
			...
			if (
				target?.closest(
					"input,textarea,select,button,a,audio,video,[role='button'],[contenteditable='true']",
				)
			)
				return;
	...
	const handleWheel = (event: React.WheelEvent<HTMLDivElement>) => {
		...
		const newScale = Math.min(Math.max(viewport.k * factor, 0.1), 3);
	...
	const handlePointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
		...
		const shouldPan =
			event.button === 1 ||
			(event.button === 0 && activeTool === "pan" && isBackgroundClick);
	...
	return (
		<div
			ref={containerRef}
			className="oc-canvas-viewport relative h-full w-full select-none overflow-hidden"
			tabIndex={0}
...
function CanvasGrid({
	viewport,
	mode,
}: { viewport: ViewportTransform; mode: CanvasBackgroundMode }) {
