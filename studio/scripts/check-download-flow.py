        ...
        with (fixture.ARTIFACTS / "download-server.log").open("w") as log:
            ...
            try:
                ...
                with sync_playwright() as pw:
                    ...
                    expect(page.get_by_role('button',name='ModelScope 下载',exact=True)).to_have_count(8)
                    ...
                    expect(dialog.get_by_placeholder('自动：跟随模型')).to_have_value('')
                    expect(dialog.locator('select').filter(has=page.locator('option[value="r"]'))).to_have_value('')
                    expect(dialog.get_by_role('button',name='保存预设',exact=True)).to_be_disabled()
